...
#define __HONEST_ENABLE_PACING_VERBOSE_LOG__ 0
namespace net {
namespace {

// The estimated system alarm granularity. // HONEST NO USE
...
#if 0 //__HONEST_ENABLE_PACING_VERBOSE_LOG__
  static uint64_t honest_sent_pkt_cnt = 0;
  HONEST_FATAL << "bytes_in_flight:" << bytes_in_flight
               << ", cwnd:" << static_cast<uint32_t>(sender_->GetCongestionWindow() / kDefaultTCPMSS)
               << ", ideal_next_packet_send_time_ -0us:" << (ideal_next_packet_send_time_ - sent_time).ToMicroseconds()
               << ", srtt:" << sender_->GetRttStats()->smoothed_rtt()
               << ", honest_sent_pkt_cnt:" << ++honest_sent_pkt_cnt;
#endif
...
QuicTime::Delta PacingSender::TimeUntilSend(
  ...
  if (ideal_next_packet_send_time_ > now + QuicTime::Delta::FromMicroseconds(QuicUtils::honest_Granularity)) {
    QUIC_DVLOG(1) << "Delaying packet: "
                  << (ideal_next_packet_send_time_ - now).ToMicroseconds();
#if __HONEST_ENABLE_PACING_VERBOSE_LOG__
    HONEST_FATAL << "Delaying packet: "
                  << (ideal_next_packet_send_time_ - now).ToMicroseconds();
#endif
...
#if __HONEST_ENABLE_PACING_VERBOSE_LOG__
  HONEST_FATAL << "Sending packet now";
#endif
