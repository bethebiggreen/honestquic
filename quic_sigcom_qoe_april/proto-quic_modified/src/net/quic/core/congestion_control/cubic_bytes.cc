...
// HONESTCHOI added it. it won't be used -> static uint32_t kDefaultNumConnections = 2;
...
QuicByteCount CubicBytes::CongestionWindowAfterAck(
  ...
  bool use_estimated_cwnd = true;
  // Cubic is "independent" of RTT, the update is limited by the time elapsed.
  if (last_congestion_window_ == current_congestion_window &&
      (event_time - last_update_time_ <= MaxCubicTimeInterval())) {
#if 1 // HONESTCHOI added it to avoid trampoline
#if 0
  HONEST_FATAL << "num_connections_:"    << num_connections_ 
               << ", use_estimated_cwnd:" << use_estimated_cwnd 
               << ", Alpha:" << Alpha() 
               << ", Beta:" << Beta() 
               << ", last_congestion_window_:" <<  last_congestion_window_
               << ", acked_bytes_count_:" << acked_bytes_count_ 
               << ", estimated_tcp_congestion_window_:" << estimated_tcp_congestion_window_;
#endif
     if(use_estimated_cwnd)
       return std::max(last_target_congestion_window_,
                       estimated_tcp_congestion_window_);
     else
       return last_target_congestion_window_;
#else
     return std::max(last_target_congestion_window_,
                     estimated_tcp_congestion_window_);
#endif
  ...
  if(use_estimated_cwnd) { // HONESTCHOI added 
