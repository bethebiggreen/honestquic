...
// HONESTCHOI added it due to reference of hoest_DefaultNumConnections
...
// HONESTCHOI added it. it won't be used -> static uint32_t kDefaultNumConnections = 2; // N-connection emulation.
...
TcpCubicSenderBase::TcpCubicSenderBase(const QuicClock* clock,
    ...
    : rtt_stats_(rtt_stats),
      ...
      num_connections_(net::QuicUtils::honest_DefaultNumConnections), // HONEST modified it due to use configured value 
      ...
      no_prr_(false) {
      HONEST_FATAL << "num_connections_:" << num_connections_
        << ", net::QuicUtils::honest_PacingRate:" << net::QuicUtils::honest_PacingRate;
}
...
QuicBandwidth TcpCubicSenderBase::PacingRate(
  ...
  // HONESTCHOI
#if 0
  float control_pace_rate = (GetCongestionWindow()+kDefaultTCPMSS)/(float)GetCongestionWindow();
  // HONEST_FATAL << "control_pace_rate:" << control_pace_rate;
  return bandwidth * (InSlowStart() ? 2 : (no_prr_ && InRecovery() ? 1 : control_pace_rate));
#else
  return bandwidth * (InSlowStart() ? 2 : (no_prr_ && InRecovery() ? 1 : net::QuicUtils::honest_PacingRate));
#endif
}
...
// HONESTCHOI added it
const RttStats* TcpCubicSenderBase::GetRttStats(void) const {
	return rtt_stats_;
}
