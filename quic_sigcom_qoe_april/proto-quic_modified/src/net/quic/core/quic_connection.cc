...
QuicConnection::QuicConnection(QuicConnectionId connection_id,
    ...
    : framer_(supported_versions,
      ...
      // sent_packet_manager_(perspective, clock_, &stats_, kCubic, kNack),
  ...
  SetMaxPacketLength(perspective_ == Perspective::IS_SERVER
                         ...
                         : kDefaultMaxPacketSize); // HONESTCHOI no longer use
#else
  SetMaxPacketLength(perspective_ == Perspective::IS_SERVER
                         ? kDefaultServerMaxPacketSize
                         : net::QuicUtils::honest_DefaultMaxPacketSize);
#endif
...
void QuicConnection::SetFromConfig(const QuicConfig& config) {
  ...
  if (config.HasClientSentConnectionOption(kMTUH, perspective_)) {
    SetMtuDiscoveryTarget(QuicUtils::honest_MtuDiscoveryTargetPacketSizeHigh);
    // SetMtuDiscoveryTarget(kMtuDiscoveryTargetPacketSizeHigh);
  }
  if (config.HasClientSentConnectionOption(kMTUL, perspective_)) {
    SetMtuDiscoveryTarget(QuicUtils::honest_MtuDiscoveryTargetPacketSizeLow);
...
//  static uint64_t num_of_delays = 0;
  ...
  if (!delay.IsZero()) {
    ...
    QUIC_DVLOG(1) << ENDPOINT << "Delaying sending " << delay.ToMilliseconds()
                  << "ms";
#if 0
    HONEST_FATAL << ENDPOINT << "Delaying sending " << delay.ToMicroseconds()
                  << "us"
