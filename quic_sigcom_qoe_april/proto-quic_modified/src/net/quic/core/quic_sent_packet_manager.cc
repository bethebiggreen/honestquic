...
#include "net/quic/core/quic_utils.h" // HONEST added it

#define __HONEST_ENABLE_VERBOSE_LOG__ 0
namespace net {
...
QuicSentPacketManager::QuicSentPacketManager(
    ...
    : unacked_packets_(),
      ...
      // using_pacing_(true), HONEST no longer use
      using_pacing_(net::QuicUtils::honest_UsingPacing), // HONEST uses it 
      use_new_rto_(false),
  ...
  HONEST_FATAL << "net::QuicUtils::honest_UsingPacing:" << net::QuicUtils::honest_UsingPacing ;
}
...
void QuicSentPacketManager::SetFromConfig(const QuicConfig& config) {
  ...
  // using_pacing_ = !FLAGS_quic_disable_pacing_for_perf_tests; // HONEST no logner use it
  using_pacing_ = net::QuicUtils::honest_UsingPacing; // HONEST uses it
  HONEST_FATAL << "HONESTCHOI using_pacing_ " << using_pacing_;
...
#if 1 // HONEST added it for breathing
//  static uint64_t honest_meaningless_cnt = 0;
//   static const uint64_t honest_magic_num = 1000000;
//   if (honest_meaningless_cnt % ++honest_magic_num)
/*
  if (rtt_updated) {
    static QuicTime::Delta prev_srtt = rtt_stats_.smoothed_rtt();
    if ((prev_srtt.ToMilleseconds() - rtt_stats_.smoothed_rtt().ToMilleseconds()) == 0)
      HONEST_FATAL << "srtt:" << rtt_stats_.smoothed_rtt().ToMicroseconds() << "us";
  }
*/
#if  __HONEST_ENABLE_VERBOSE_LOG__
  if (rtt_updated)
	  HONEST_FATAL << "srtt:" << rtt_stats_.smoothed_rtt().ToMicroseconds() << "us";
#endif
#endif
...
#if 1                                                                                                                                                    
#if 0
  HONEST_FATAL << "est_bw:" << BandwidthEstimate() 
    << ", srtt:" << rtt_stats_.smoothed_rtt().ToMilliseconds() << "ms" 
    << ", cwnd:" << GetCongestionWindowInTcpMss();                                                                                                     
#else
#endif
...
bool QuicSentPacketManager::OnPacketSent(
  ...
  if (using_pacing_) {
#if 0
  HONEST_FATAL << "bytes_in_flight:" << unacked_packets_.bytes_in_flight()
	       << ", cwnd:" << GetCongestionWindowInTcpMss()       
	       << ", srtt:" << rtt_stats_.smoothed_rtt().ToMicroseconds() << "us" 
<< ", ideal_next_packet_send_time_:" << (pacing_sender_.ideal_next_packet_send_time_ - QuicTime::Zero()).ToMicroseconds() << "us"
<< ", now:" << (clock_->Now() - QuicTime::Zero()).ToMicroseconds() << "us";
               //<< ", ideal_next_packet_send_time_ :" << pacing_sender_.ideal_next_packet_send_time_
#endif
...
void QuicSentPacketManager::RetransmitRtoPackets() {
  ...
  for (QuicUnackedPacketMap::const_iterator it = unacked_packets_.begin();
    ...
    if (it->retransmittable_frames.empty() && it->in_flight &&
      ...
      // HONEST added it for debugging
      HONEST_FATAL << "LOSS RTO occurred!" 
	           << " total:" << stats_->packets_lost
...
void QuicSentPacketManager::InvokeLossDetection(QuicTime time) {
  ...
  for (const auto& pair : packets_lost_) {
    ...
    // HONEST added it for debugging
    HONEST_FATAL << "LOSS RET occurred!"
            << " total:" << stats_->packets_lost
	    << ", srtt:" << rtt_stats_.smoothed_rtt().ToMicroseconds() << "us" 
	    << ", cwnd:" << GetCongestionWindowInTcpMss();       
